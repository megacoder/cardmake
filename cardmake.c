...
)
{
	...
	while( !feof(ascii_fd)) {
		...
		while (src_col <= max_col) {
			...
			if (table == 1) {
				even_col = o26_comm_code[ (unsigned) even_ch ];
				odd_col = o26_comm_code[ (unsigned) odd_ch ];
			} else if (table == 2) {
				even_col = o26_ftn_code[ (unsigned) even_ch ];
				odd_col = o26_ftn_code[ (unsigned) odd_ch ];
			} else if (table == 4) {
				even_col = o29_code[ (unsigned) even_ch ];
				odd_col = o29_code[ (unsigned) odd_ch ];
			} else { /* table == 8 */
				even_col = EBCDIC_code[ (unsigned) even_ch ];
				odd_col = EBCDIC_code[ (unsigned) odd_ch ];
			}
